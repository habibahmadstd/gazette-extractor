    ...
    with pdfplumber.open(io.BytesIO(file_bytes)) as pdf:
        ...
        for i, page in enumerate(pdf.pages):
            ...
            progress.progress(pct,
                text=f"Page {i+1}/{total} — Found {len(matched_records)}, "
                     f"still looking for {len(remaining)}…")

            # Text extraction (light on memory)
            try:
            ...
            # Stop early once all found
            if not remaining:
                status.success(f"✅ All roll numbers found at page {i+1}/{total}!")
                break
...
with st.sidebar:
    ...
    cols_per_student = st.number_input(
        ...
        max_value=20,
        value=5,
        help="How many columns make up one student's record. E.g. Roll No, Name, blank, blank, Marks = 5",
    )
    st.markdown("---")
    st.markdown("**Tip:** If results look wrong, try changing this number (e.g. 6 or 7).")
...
if st.button("🔍 Extract Results", type="primary", use_container_width=True):
    ...
    if not roll_numbers:
        st.error("No roll numbers found. Check the first column of your file.")
        st.stop()
    ...
    if result_df.empty:
        st.warning(
            ...
            "Check roll numbers are correct and 'Columns per student' matches your gazette."
        )
    else:
